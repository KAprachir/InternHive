...
export default function InternshipCard({ internship, matchScore }: InternshipCardProps) {
  ...
  // Premium fallback illustrations/gradients based on the internship's category
  const getFallbackImage = (category: string = ''): string => {
    ...
    if (cleanCategory.includes('software') || cleanCategory.includes('dev') || cleanCategory.includes('code')) {
      return 'https://images.unsplash.com/photo-1605379399642-870262d3d051?auto=format&fit=crop&w=600&q=80';
    }
    if (cleanCategory.includes('design') || cleanCategory.includes('ui') || cleanCategory.includes('ux') || cleanCategory.includes('creative')) {
      return 'https://images.unsplash.com/photo-1561070791-26c113006238?auto=format&fit=crop&w=600&q=80';
    }
  ...
  const cardImage = internship.imageUrl || getFallbackImage(internship.category);

  return (
    <div className="bg-white rounded-[24px] overflow-hidden shadow-[0px_4px_20px_rgba(20,33,61,0.05)] hover:shadow-[0px_8px_30px_rgba(20,33,61,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full border border-gray-100/50">
      <div>
        {/* Card Banner Image */}
        <div className="relative h-40 w-full overflow-hidden bg-gray-100">
          ...
          {internship.category && (
            <span className="absolute top-3 left-3 bg-[#14213D] text-white text-[10px] font-bold px-2.5 py-1 rounded-md shadow-sm">
              {internship.category}
            </span>
          )}
        </div>
        ...
        <div className="p-6">
          ...
          {/* Title */}
          <h4 className="text-md font-bold text-[#14213D] mt-2 mb-3 leading-snug font-[family-name:var(--font-space-grotesk)] line-clamp-1">
            {internship.title}
          </h4>
      ...
      <div className="px-6 pb-6 pt-4 border-t border-gray-100 flex items-center justify-between gap-4">
        <div>
